...
public class Indexer {
    ...
    HashMap<Integer, ArrayList<String>> docTokens = new HashMap<Integer, ArrayList<String>>();
    HashMap<Integer, HashMap<String, Integer>> docTF = new HashMap<Integer, HashMap<String,Integer>>();
            ...
            if ( f.canRead() ) {
                if ( f.isDirectory() ) {
                    ...
                    try {
                        ...
                        docTF.put(docID, new HashMap<String, Integer>());
                        while ( tok.hasMoreTokens() ) {
                            ...
                            if (!tokens.contains(token)) {
                                tokens.add(token);
                                docTF.get(docID).put(token, 1);
                            } else {
                                int occur = docTF.get(docID).get(token)+1;
                                docTF.get(docID).put(token, occur);
                            }
                        }
